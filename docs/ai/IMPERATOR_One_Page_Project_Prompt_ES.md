...
El proyecto esta en **Phase 2 Platform Foundation en ejecucion**. La persistencia PostgreSQL esta certificada, existe un runtime web Spring Boot ejecutable y Sprint 2.8.1 ha cerrado el contrato de errores REST y correlacion HTTP. El siguiente gate autorizado es **Sprint 2.8.2 - Evidence Import Route Shell**. El contexto de Fase 1 y los contratos 31-40 permanecen como autoridad congelada, complementados por `docs/decisions/14_Decision_Log.md` y el estado operativo de `agents/phase2/README.md`. No debe crear codigo, servicios, conectores reales, infraestructura ni APIs fuera del sprint/modulo expresamente autorizado por el fundador.
