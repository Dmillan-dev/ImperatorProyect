...
### [2026-08-01] D084 - Sprint 3.4 Runtime Certification Deferral
- Decision: accept an explicit process exception for Sprint 3.4. Its implementation is `COMPLETE`, while its PostgreSQL 18.2 runtime certification is `DEFERRED` because the required certification environment was unavailable to the authorized command. Sprint 3.5 may proceed. The deferred certification must pass before Pilot Readiness, MVP closure or the first production release.
- Rationale: the Java 21 offline implementation gate completed with 88 passing tests and no known implementation defect was identified by the checks that executed. The explicit `mvnw.cmd -Ppostgresql-integration clean verify` attempt stopped at Maven toolchain selection because that invoked process exposed only Java 17; PostgreSQL, Flyway and Failsafe tests never ran. Treating this environmental limitation as either a product defect or an accidental pending gate would misstate the evidence, while silently claiming certification would be false. A named deferral preserves delivery sequence and makes the unresolved obligation auditable.
- Impact: active control documents must use `DEFERRED`, not `FAILED`, `PENDING` or `NOT CERTIFIED`, for the Sprint 3.4 runtime gate. This exception is not a certification pass and creates no PostgreSQL, Flyway, schema, dependency or architecture claim. Before Pilot Readiness, MVP closure or the first production release, the project must execute `mvnw.cmd -Ppostgresql-integration clean verify` with Java 21, PostgreSQL 18.2 and the required `IMPERATOR_IT_*` configuration, then record the actual result. D084 changes sequencing only and authorizes no Java, SQL, test or runtime implementation change.

### [2026-08-01] D085 - MVP Delivery Roadmap Evolution
- Decision: accept Sprint 3.6 Basic Java CI as certified and supersede only the unexecuted post-CI delivery order in D079. Freeze the remaining gates as Sprint 3.7 - Functional REST API, Sprint 3.8 - JWT Authentication, Sprint 3.9 - RBAC Authorization, Sprint 4.0 - GitHub Integration, Sprint 4.1 - AWS Integration, Sprint 4.2 - Executive Dashboard, Sprint 4.3 - Docker Production Runtime, Sprint 4.4 - Observability, Sprint 4.5 - Pilot Readiness and Sprint 5.0 - MVP Release. Sprint 3.7 is the sole next authorized gate; every later gate remains pending until separately authorized and accepted.
- Rationale: the deterministic `DRC-AOA-001` business-value loop, runtime composition, REST route topology and trustworthy Java CI now exist. The shortest controlled path to product evidence is to expose the already implemented Application capabilities through the frozen REST surface, secure that surface before external use, connect real sources only after authentication and authorization, then add the bounded user surface and operational readiness required for a pilot and release. This order separates functional API delivery, authentication, authorization, integrations, presentation and operations into auditable gates.
- Impact: D085 changes roadmap order only. Sprint 3.7 may replace existing `501` route-shell behavior through existing Application input ports, but it may not add routes or expose the runtime to real customer data before Sprints 3.8 and 3.9 pass. Sprints 4.0 and 4.1 authorize future connector gates, not connector implementation now. For Sprint 4.2, `Executive Dashboard` means the frozen thin, single-case Decision Review and Business Value surface; it does not authorize the broader multi-case Executive Workspace, portfolio dashboard or new product model prohibited by the MVP contracts. D079 remains authoritative for the completed vertical-slice acceleration and all unaffected boundaries. D084 runtime certification remains mandatory before Sprint 4.5, Sprint 5.0 or production. D085 authorizes no Java, SQL, Flyway, test, dependency, REST, Domain, decision-contract or business-logic change.

### [2026-08-01] D086 - Functional REST Application Contract
- Decision: add `docs/architecture/45_Functional_REST_Application_Contract.md` as the frozen contract for Sprint 3.7. Narrowly supersede D085's restriction to existing Application input ports by authorizing exactly eight route-aligned read-only query input ports and use cases plus one light-CQRS `MvpReadModelQueryPort`. Freeze explicit REST DTO mapping, `GET /api/v1/business-value?decisionId={id}` semantics, temporary local trusted-actor headers, idempotency mapping and typed HTTP error handling. Preserve the existing 15-route topology and all existing command use-case signatures.
- Rationale: implementation evidence showed that eight frozen GET routes had no inbound query boundary, the existing Business Value port required a Decision selector absent from its route shell, and governance commands required actor identity before JWT exists. Calling write repositories from controllers, inventing routes or trusting actor fields in command bodies would violate Hexagonal Architecture and create a breaking security contract. A frozen CQRS read boundary and replaceable local actor source close those gaps without moving business authority into REST.
- Impact: Sprint 3.7 may add the exact query contracts, Application projections, read-only PostgreSQL query adapter, REST DTOs/mappers/controllers, typed exception mappings and runtime wiring defined by Document 45. Write repositories, Domain, V1, Flyway, dependencies and existing command use cases remain unchanged. `CreateDecisionInputPort` and `GenerateRecommendationInputPort` remain internal and receive no REST route. Trusted actor mode is disabled by default, forbidden for external or real-customer use and must be replaced by verified JWT claims in Sprint 3.8; permanent route authorization remains Sprint 3.9. D086 itself authorizes no implementation file, test, SQL, controller or runtime change.

### [2026-08-02] D087 - JWT Authentication Contract
- Decision: add `docs/architecture/46_JWT_Authentication_Contract.md` as the frozen Sprint 3.8 authentication authority. IMPERATOR is an OAuth2 Resource Server only and must use Spring Security's JWT support for every `/api/v1/**` request. Accept only RS256 JWTs from an externally configured issuer and HTTPS JWKS endpoint, require exact audience `imperator-api`, canonical UUID `sub`, required `exp`, validated optional `nbf`, one `imperator_role` from `ADMIN`, `PLATFORM_ENGINEER`, `FINANCE` or `AUDITOR`, and a mandatory JOSE `kid`. Narrowly supersede only D086's temporary trusted-actor mechanism; all D086 routes, DTOs, Application boundaries, statuses and transaction contracts remain frozen.
- Rationale: Sprint 3.7 certified a functional API but its temporary actor headers are deliberately not authentication and would become a critical bypass if retained. Freezing resource-server responsibility, claim translation, signature and key validation, key rotation, fail-closed behavior, stateless HTTP handling and the security error envelope before implementation prevents a custom JWT stack, implicit identity authority, unsafe fallback or accidental RBAC expansion. Framework-managed JWKS caching and bounded refresh support key rotation without a custom cache; a missing or unknown `kid`, failed refresh or unverifiable signature always returns a safe `401`.
- Impact: Sprint 3.8 may add only the Spring Boot OAuth2 Resource Server dependency and optional test-scope Spring Security test support, create the API security adapter and bootstrap wiring, remove `TrustedActorContextResolver` and its property, add safe `INVALID_TOKEN` handling, and update tests to use signed test JWTs. Trusted actor headers become inert and can neither authenticate nor override JWT claims. Sprint 3.8 authenticates callers but introduces no route-, method-, evidence- or sensitivity-level RBAC; those remain exclusively Sprint 3.9. It adds no Authorization Server, login, users, passwords, refresh tokens, tenant behavior, routes, Domain change, business rule, PostgreSQL object, Flyway migration, connector, frontend or external exposure. Certification requires Java 21 plus the full PostgreSQL 18.2 integration profile and proof that persisted governance actor identity originates from the validated JWT.
